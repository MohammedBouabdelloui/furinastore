    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
      ...
      <li>
        <div class="flex items-center">
          ...
          <a href="{{ route('dashboard.topup.edit', $topup) }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-blue-700">Edit</a>
        </div>
      </li>
    </ol>
</nav>
...
<form dir="rtl" action="{{ route('dashboard.topup.update', $topup) }}" method="POST" class="max-w-[90%] mx-auto my-8 bg-white rounded-2xl p-8 font-cairo" enctype="multipart/form-data">

    @csrf
    @method('PUT')

    <h1 class="text-3xl font-bold text-gray-900 mb-6">عدل على معلومات منتوجا من نوع عمليات الشحن</h1>

    <div class="mb-6">
      <label for="title" class="block mb-3 text-sm font-medium text-gray-500 ">عنوان المنتوج</label>
      <input type="text" id="title" name="title" value="{{ $topup->title }}" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 outline-none focus:ring-0" placeholder="8080 كرستالة" required />
    </div>
    ...
    <div class="mb-6">
      ...
      <input type="number" min="0" id="topup_value" value="{{ $topup->topup_value }}" name="topup_value" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 outline-none focus:ring-0" placeholder="120" required />
    </div>
    ...
    <div class="mb-6">
        ...
        <input type="file" id="topup_picture" name="picture" value="{{ $topup->picture }}" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 outline-none  focus:border-transparent focus:ring-0" placeholder="120" required />
    </div> 
